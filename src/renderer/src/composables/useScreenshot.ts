...
export function useScreenshot(deps: {
  ...
  function getScreenshotInputPath(): string {
    return resolvePlayablePath(deps.currentFile.value);
  }
  ...
  async function captureByTime(): Promise<void> {
    if (!screenshotTimeInput.value || capturing.value) {
      return;
    }
  ...
  async function batchCapture(): Promise<void> {
    ...
    if (!dur || !batchInterval.value || batchInterval.value < 1 || capturing.value) {
      return;
    }
