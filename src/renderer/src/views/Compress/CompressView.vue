...
watch([crfValue, resolution, bitrate, codec, audioBitrate, preset, nvencPreset, twoPass], () => {
  if (!bitrate.value || isGpuEncoder.value) {
    twoPass.value = false;
  }
  savePresetDebounced();
});
...
function estimateOutputSize(entry: FileEntry): string {
  ...
  const codecFactor =
    (codec.value || '').includes('265') || (codec.value || '').includes('hevc') || isVp9.value ? 0.7 : 1.0;
...
async function startCompress(): Promise<void> {
  ...
  try {
    const batchFiles = snapshot.map((f) => ({
      input: f.path,
      output: f.outputPath,
      crf: crfValue.value,
      resolution: resolution.value,
